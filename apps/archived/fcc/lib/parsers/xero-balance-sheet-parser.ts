...
interface BalanceSheetStructure {
  assets: {
    fixed: ParsedAccount[]
    current: ParsedAccount[]
    totalFixed: number
    totalCurrent: number
    totalAssets: number
  }
...
export class XeroBalanceSheetParser {
  /**
   ...
   */
  static parse(data: string[][] | any[], targetDate?: string): BalanceSheetStructure {
    ...
    return this.parseCSVData(data as string[][], targetDate)
  }
   ...
   */
  private static parsePreformattedData(data: any[]): BalanceSheetStructure {
    const result: BalanceSheetStructure = {
      assets: {
        fixed: [],
        current: [],
        totalFixed: 0,
    ...
    data.forEach(row => {
      ...
      if (!accountName || balance === 0) return
    ...
    structuredLogger.info('[XeroBalanceSheetParser] Parsed pre-formatted data', {
   ...
   */
  private static parseCSVData(csvData: string[][], targetDate?: string): BalanceSheetStructure {
    const result: BalanceSheetStructure = {
      assets: {
        fixed: [],
        current: [],
    ...
    if (targetDate && csvData.length > 0) {
      ...
      structuredLogger.info('[XeroBalanceSheetParser] Date column detection', {
        targetDate,
        dateColumnIndex,
        headers: csvData[0]
      })
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      if (firstCell.startsWith('Total Fixed Assets')) {
        ...
        structuredLogger.info('[XeroBalanceSheetParser] Found total fixed assets', { 
        ...
        structuredLogger.info('[XeroBalanceSheetParser] Found total current assets', { 
          value: account.balance,
          willRecalculate: account.balance === 0
        })
      } else if (firstCell.startsWith('Total Assets')) {
        result.assets.totalAssets = account.balance
        structuredLogger.info('[XeroBalanceSheetParser] Found total assets', { 
          value: account.balance,
          willRecalculate: account.balance === 0
        })
        ...
        structuredLogger.info('[XeroBalanceSheetParser] Found total current liabilities', { 
        ...
        structuredLogger.info('[XeroBalanceSheetParser] Found total non-current liabilities', { 
          originalValue: account.balance,
          willRecalculate: account.balance === 0
        })
      } else if (firstCell.startsWith('Total Liabilities')) {
        result.liabilities.totalLiabilities = account.balance
        structuredLogger.info('[XeroBalanceSheetParser] Found total liabilities', { 
          originalValue: account.balance,
          willRecalculate: account.balance === 0
        })
        ...
        structuredLogger.info('[XeroBalanceSheetParser] Found total equity', { 
          value: account.balance,
          willRecalculate: account.balance === 0
        })
      } else if (firstCell === 'Net Assets' || firstCell === 'Net Worth') {
        result.netAssets = account.balance
        structuredLogger.info('[XeroBalanceSheetParser] Found net assets', { 
          value: account.balance,
          willRecalculate: account.balance === 0
        })
      } else {
    ...
    if (result.assets.totalFixed === 0 && result.assets.fixed.length > 0) {
      ...
      structuredLogger.info('[XeroBalanceSheetParser] Calculated fixed assets total from accounts', {
    ...
    if (result.assets.totalCurrent === 0 && result.assets.current.length > 0) {
      result.assets.totalCurrent = result.assets.current
        .filter(a => !a.isSubTotal)
        .reduce((sum, account) => sum + account.balance, 0)
      structuredLogger.info('[XeroBalanceSheetParser] Calculated current assets total from accounts', {
    ...
    if (result.liabilities.totalCurrent === 0 && result.liabilities.current.length > 0) {
      result.liabilities.totalCurrent = result.liabilities.current
        .filter(a => !a.isSubTotal)
        .reduce((sum, account) => sum + account.balance, 0)
      structuredLogger.info('[XeroBalanceSheetParser] Calculated current liabilities total from accounts', {
    ...
    if (result.liabilities.totalNonCurrent === 0 && result.liabilities.nonCurrent.length > 0) {
      result.liabilities.totalNonCurrent = result.liabilities.nonCurrent
        .filter(a => !a.isSubTotal)
        .reduce((sum, account) => sum + account.balance, 0)
      structuredLogger.info('[XeroBalanceSheetParser] Calculated non-current liabilities total from accounts', {
    ...
    if (result.equity.totalEquity === 0 && result.equity.accounts.length > 0) {
      result.equity.totalEquity = result.equity.accounts
        .filter(a => !a.isSubTotal)
        .reduce((sum, account) => sum + account.balance, 0)
      structuredLogger.info('[XeroBalanceSheetParser] Calculated equity total from accounts', {
    ...
    if (result.assets.totalAssets === 0) {
      result.assets.totalAssets = result.assets.totalFixed + result.assets.totalCurrent
      structuredLogger.info('[XeroBalanceSheetParser] Calculated total assets', {
        totalAssets: result.assets.totalAssets,
    ...
    if (result.liabilities.totalLiabilities === 0) {
      result.liabilities.totalLiabilities = result.liabilities.totalCurrent + result.liabilities.totalNonCurrent
      structuredLogger.info('[XeroBalanceSheetParser] Calculated total liabilities', {
        totalLiabilities: result.liabilities.totalLiabilities,
    ...
    if (result.netAssets === 0) {
      result.netAssets = result.assets.totalAssets - result.liabilities.totalLiabilities
      structuredLogger.info('[XeroBalanceSheetParser] Calculated net assets', {
        netAssets: result.netAssets,
        totalAssets: result.assets.totalAssets,
        totalLiabilities: result.liabilities.totalLiabilities
      })
    ...
    structuredLogger.info('[XeroBalanceSheetParser] Parsed CSV balance sheet', {
      totalAssets: result.assets.totalAssets,
      totalLiabilities: result.liabilities.totalLiabilities,
      netAssets: result.netAssets,
      accountCounts: {
  ...
  private static parseAccountLine(row: string[], dateColumnIndex: number | null = null): ParsedAccount | null {
    ...
    if (dateColumnIndex !== null && dateColumnIndex < row.length) {
      ...
      if (cellValue) {
        ...
        structuredLogger.debug('[XeroBalanceSheetParser] Parsing account with date column', {
          accountName,
          dateColumnIndex,
          cellValue,
   ...
   */
  private static findDateColumnIndex(csvData: string[][], targetDate: string): number | null {
    ...
    for (let rowIndex = 0; rowIndex < Math.min(10, csvData.length); rowIndex++) {
      ...
      for (let colIndex = 0; colIndex < row.length; colIndex++) {
        ...
        if (cleanedCell === targetDate || 
          ...
          structuredLogger.info('[XeroBalanceSheetParser] Found target date column', {
            targetDate,
    ...
    structuredLogger.warn('[XeroBalanceSheetParser] Could not find target date column', {
      targetDate,
   ...
   */
  private static normalizeDateString(dateStr: string | any): string {
    if (!dateStr) return ''
  ...
  private static parseAmount(value: string): number {
    ...
    if (isNegative || (result !== 0 && Math.abs(result) > 0.01)) {
      structuredLogger.debug('[XeroBalanceSheetParser] Parsed amount', {
        original: strValue,
        cleaned,
        isNegative,
        result
   ...
   */
  static toImportFormat(parsed: BalanceSheetStructure, reportDate: Date) {
    // Find specific accounts by name patterns
    const findAccount = (accounts: ParsedAccount[], patterns: string[]): number => {
      let total = 0
      ...
      accounts.forEach(account => {
        ...
        for (const pattern of patterns) {
          if (accountNameLower.includes(pattern.toLowerCase())) {
            ...
            structuredLogger.debug('[XeroBalanceSheetParser] Found matching account', {
              pattern,
    ...
    structuredLogger.info('[XeroBalanceSheetParser] Cash detection', {
      totalCash: cash,
    ...
    return {
      assets: {
        current: {
          ...
          accounts: parsed.assets.current.map(acc => ({
            name: acc.name,
            balance: acc.balance,
            code: acc.code
          }))
        ...
        nonCurrent: {
          ...
          // Include individual accounts
          accounts: parsed.assets.fixed.map(acc => ({
            name: acc.name,
            balance: acc.balance,
            code: acc.code
          }))
      ...
      liabilities: {
        current: {
          ...
          // Include individual accounts
          accounts: parsed.liabilities.current.map(acc => ({
            name: acc.name,
            balance: acc.balance,
            code: acc.code
          }))
        ...
        nonCurrent: {
          ...
          // Include individual accounts
          accounts: parsed.liabilities.nonCurrent.map(acc => ({
            name: acc.name,
            balance: acc.balance,
            code: acc.code
          }))
      ...
      equity: {
        ...
        // Include individual accounts
        accounts: parsed.equity.accounts.map(acc => ({
          name: acc.name,
          balance: acc.balance,
          code: acc.code
        }))
