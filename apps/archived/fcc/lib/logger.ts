      ...
      format: winston.format.combine(
        ...
        winston.format.printf(
          (info) => {
            ...
            if (typeof message === 'string' && message.startsWith('[CLIENT]')) {
              ...
              const moduleMatch = typeof message === 'string' ? message.match(/^\[([^\]]+)\]/) : null;
              if (moduleMatch) {
                moduleName = moduleMatch[1];
                cleanMessage = typeof message === 'string' ? message.substring(moduleMatch[0].length).trim() : '';
              }
            }
...
// Use a global flag to prevent multiple initialization messages
const LOGGER_INIT_KEY = '__WINSTON_LOGGER_INITIALIZED__';

if (process.env.NODE_ENV !== 'production' && typeof window === 'undefined') {
  // Check if we've already initialized using a global flag
  if (!(global as any)[LOGGER_INIT_KEY]) {
    // Mark as initialized globally
    (global as any)[LOGGER_INIT_KEY] = true;
