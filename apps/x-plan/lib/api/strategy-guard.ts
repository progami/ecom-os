...
export async function requireXPlanStrategyAccess(strategyId: string | null | undefined, session: Session) {
...
export async function requireXPlanStrategiesAccess(strategyIds: Array<string | null | undefined>, session: Session) {
  const actor = getStrategyActor(session)
  const unique = Array.from(
    new Set(strategyIds.filter((strategyId): strategyId is string => typeof strategyId === 'string' && strategyId.length > 0)),
  )
