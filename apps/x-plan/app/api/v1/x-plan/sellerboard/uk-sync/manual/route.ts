...
export const POST = withXPlanAuth(async (_request: Request, session) => {
  ...
  try {
    ...
    const actualSalesResult = await syncSellerboardUkActualSales({ reportUrl: ordersReportUrl });
    const actualSales = {
      ok: true,
    ...
    const dashboardResult = await syncSellerboardUkDashboard({ reportUrl: dashboardReportUrl });
    const dashboard = {
      ok: true,
