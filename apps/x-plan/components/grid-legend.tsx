...
const LEGEND_ITEMS = [
  {
    ...
    swatchClass: 'bg-cyan-600 dark:bg-[#00C2B9]',
  },
  {
    id: 'calculated',
    label: 'Calculated output',
    description: 'Updates from workbook logic',
    swatchClass: 'bg-slate-500 dark:bg-[#6F7B8B]',
  },
  {
    id: 'active',
    label: 'Linked row',
...
export function GridLegend({ className }: GridLegendProps) {
  const baseClass =
    'flex flex-wrap items-center gap-x-6 gap-y-2 text-xs text-slate-700 dark:text-slate-200/80'
        ...
        <div key={item.id} className="flex items-center gap-2">
          ...
          {item.description ? (
            <>
              ...
              <span aria-hidden className="hidden text-slate-600 sm:inline dark:text-slate-400">
                · {item.description}
              </span>
            </>
          ) : null}
