...
export function ProductSetupGrid({ products, className }: ProductSetupGridProps) {
  ...
  return (
    <section
      className={clsx(
        'relative space-y-6 rounded-3xl border border-slate-200] bg-white] p-6 text-slate-900 shadow-lg)] ring-1 ring-slate-200]/60',
    ...
    >
      <div className="relative flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-700">Catalogue</p>
          <h2 className="text-2xl font-semibold text-slate-900">Product roster</h2>
        ...
        <div className="flex flex-col items-start gap-3 lg:items-end">
          {isAdding ? (
            <form
              className="flex w-full min-w-[260px] flex-col gap-3 rounded-2xl border border-slate-300 bg-slate-50 p-4 shadow-md)] lg:w-auto"
              onSubmit={(event) => {
            ...
            >
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="flex flex-col gap-1 text-left">
                  <span className="text-xs font-semibold uppercase tracking-[0.32em] text-cyan-700">SKU</span>
                  <input
                    value={creatingSku}
                    onChange={(event) => setCreatingSku(event.target.value)}
                    placeholder="e.g. CS-007"
                    className="w-full rounded-lg border border-slate-300 bg-white]/90 px-3 py-2 text-sm text-slate-900 placeholder-slate-500 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/70"
                  />
                </label>
                <label className="flex flex-col gap-1 text-left">
                  <span className="text-xs font-semibold uppercase tracking-[0.32em] text-cyan-700">Product name</span>
                  <input
                    value={creatingName}
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                <button
                  ...
                  disabled={isCreating}
                  className="rounded-lg border border-slate-300 px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-slate-700 transition hover:border-cyan-500 hover:text-cyan-700 disabled:cursor-not-allowed disabled:opacity-60"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isCreating}
                  className="rounded-lg bg-cyan-600] px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-white] transition hover:bg-cyan-700] disabled:cursor-not-allowed disabled:opacity-60"
                >
            ...
            <button
              ...
              className="inline-flex items-center rounded-lg bg-cyan-600] px-4 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-white] shadow-md)] transition hover:bg-cyan-700]"
            >
              New product
            </button>
          )}
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-300 bg-white">
        <table className="min-w-full divide-y divide-slate-200 text-sm text-slate-900">
          <thead className="bg-slate-100 text-xs uppercase tracking-[0.28em] text-cyan-700">
            <tr>
                    ...
                    <td className="px-4 py-3 align-top">
                      {isEditing ? (
                        <input
                          ...
                          className="w-full rounded-lg border border-cyan-400/50 bg-white]/95 px-3 py-2 text-sm text-slate-900 placeholder-slate-500 focus:border-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300/70"
                        />
                      ) : (
                        <span className="font-semibold tracking-wide text-slate-900">{row.sku || '—'}</span>
                      )}
                    </td>
                    <td className="px-4 py-3 align-top">
                      {isEditing ? (
                        ...
                        {isEditing ? (
                          <>
                            <button
                              ...
                              disabled={isSaving}
                              className="rounded-lg bg-cyan-600] px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.24em] text-white] transition enabled:hover:bg-cyan-700] disabled:cursor-not-allowed disabled:opacity-60"
                            >
                            ...
                            <button
                              ...
                              className="rounded-lg border border-slate-300 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.24em] text-slate-700 transition hover:border-cyan-500 hover:text-cyan-700"
                            >
                              Cancel
                            </button>
                          </>
                          ...
                          <>
                            ...
                            <button
                              ...
                              className="rounded-lg border border-rose-400/50 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.24em] text-rose-600 transition enabled:hover:border-rose-500 enabled:hover:bg-rose-50 enabled:hover:text-rose-700 disabled:cursor-not-allowed disabled:opacity-60"
                            >
