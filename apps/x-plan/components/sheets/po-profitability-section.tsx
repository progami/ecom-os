...
export function POProfitabilitySection({
  ...
  const { maxValue } = useMemo(() => {
    if (filteredData.length === 0) return { maxValue: 1 }
    const maxRevenue = Math.max(...filteredData.map((po) => po.grossRevenue), 1)
    return { maxValue: maxRevenue * 1.1 }
  }, [filteredData])
            ...
            >
              <defs>
                <linearGradient id={`${gradientId}-revenue`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#0891b2" stopOpacity={0.8} className="dark:[stop-color:#00C2B9]" />
                  <stop offset="100%" stopColor="#0891b2" stopOpacity={0.3} className="dark:[stop-color:#00C2B9]" />
                </linearGradient>
                <linearGradient id={`${gradientId}-profit`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#059669" stopOpacity={0.8} className="dark:[stop-color:#10b981]" />
                  <stop offset="100%" stopColor="#059669" stopOpacity={0.3} className="dark:[stop-color:#10b981]" />
                </linearGradient>
              </defs>
