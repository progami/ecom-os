        ...
        <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-3 px-4 py-3 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-col gap-1">
              ...
              <h1 className="text-lg font-semibold text-slate-900">{sheetTitle}</h1>
              <p className="max-w-3xl text-xs text-slate-500">{sheetDescription}</p>
            </div>
            {actions && <div className="flex items-center gap-2">{actions}</div>}
          </div>
          {metaItems.length > 0 && <p className="text-xs text-slate-400">{metaItems.join(' • ')}</p>}
        </div>
      </header>
