...
async function importProductSetup(workbook: XLSX.WorkBook, prisma: PrismaClient) {
  ...
  const productSection = leadHeaderIndex >= 0 ? matrix.slice(0, leadHeaderIndex) : matrix

  const productRows = productSection.filter(
    (row) =>
      row?.[0] &&
      row[0] !== 'Product Name' &&
      row[0] !== 'PRODUCT CONFIGURATION' &&
      row[0] !== 'LEAD TIME CONFIGURATION (WEEKS)' &&
      row[0] !== 'BUSINESS PARAMETERS' &&
      typeof row[1] === 'number'
  )
