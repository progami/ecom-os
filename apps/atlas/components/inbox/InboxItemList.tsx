...
function getPriorityConfig(item: WorkItemDTO): { dot: string; label?: string } {
  // Unified scheme: neutral backgrounds, colored left bar only
  if (item.isOverdue) {
    return {
      dot: 'bg-red-500',
  ...
  if (item.priority === 'URGENT' || item.priority === 'HIGH') {
    return {
      dot: 'bg-amber-500',
      label: item.priority === 'URGENT' ? 'Urgent' : undefined,
    }
  }
  return {
