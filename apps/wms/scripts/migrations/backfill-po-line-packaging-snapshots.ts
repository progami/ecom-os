...
Usage:
  pnpm --filter @ecom-os/wms tsx scripts/migrations/backfill-po-line-packaging-snapshots.ts [options]
...
async function applyForSchema(
  ...
  if (!hasSnapshotColumns) {
    console.log(`${banner} missing snapshot columns; run ensure-wms-tenant-schema first`)
    await client.query('ROLLBACK')
    return
  }
