  ...
  const guideSteps: Record<string, GuideStep[]> = {
    ...
    staff: [
      ...
      },
      {
        ...
        icon: FileText,
        link: '/finance/warehouse-invoices'
      },
      {
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-foreground">Get started with your warehouse management tasks</p>
          </div>
        </div>
        <button
            ...
            <Component
              ...
              className={`bg-card p-4 rounded-xl border ${
                isDisabled
                  ...
                  : 'border-border hover:border-cyan-300 hover:shadow-soft transition-all group cursor-pointer'
              }`}
            >
              <div className="flex items-start gap-3">
                <div className={`p-2 rounded-lg ${
                  isDisabled ? 'bg-muted' : 'bg-cyan-100 group-hover:bg-cyan-200 transition-colors'
                }`}>
                  <step.icon className={`h-5 w-5 ${isDisabled ? 'text-muted-foreground' : 'text-cyan-600'}`} />
                </div>
                <div className="flex-1">
                  <h4 className={`font-medium text-sm mb-1 ${
                    isDisabled ? 'text-muted-foreground' : 'group-hover:text-cyan-600 transition-colors'
                  }`}>
                    {step.title}
                  </h4>
                ...
                {!isDisabled && (
                  <ArrowRight className="h-4 w-4 text-muted-foreground group-hover:text-cyan-600 transition-colors mt-0.5" />
                )}
              </div>
            </Component>
          )
