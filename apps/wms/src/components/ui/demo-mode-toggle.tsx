...
export function DemoModeToggle({ onDataChange }: DemoModeToggleProps) {
  ...
  const handleGenerateDemoData = async () => {
    ...
    try {
      ...
      toast.error(_error instanceof Error ? _error.message : 'Failed to generate demo data')
    } finally {
  ...
  const handleClearDemoData = async () => {
    ...
    try {
      ...
      toast.error(_error instanceof Error ? _error.message : 'Failed to clear demo data')
    } finally {
      setIsClearing(false)
      setIsOpen(false)
    }
              ...
              <div className="space-y-3">
                {!isDemoMode ? (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-cyan-600 to-brand-teal-600 text-white rounded-lg font-medium hover:from-cyan-700 hover:to-brand-teal-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isGenerating ? (
                      <>
