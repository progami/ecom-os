...
export function PermissionBadge({ permission, granted = true, className }: PermissionBadgeProps) {
  ...
  const getResourceColor = (resource: string) => {
    ...
    return colors[resource] || 'bg-muted text-foreground border-border'
  }
  ...
  return (
    <Badge
      ...
      className={cn(
        'text-xs font-medium',
        granted ? getResourceColor(resource) : 'bg-muted text-muted-foreground border-border line-through',
        className
      )}
    >
      {resource}:{action}
