  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex min-h-full items-center justify-center p-4">
        ...
        <div className="relative bg-card rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
          {/* Header */}
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
                ...
                {validation.details.dependentTransactions.length > 5 ? (
                  ...
                  <div className="space-y-3">
                    {/* Summary Stats */}
                    <div className="bg-card rounded border border-amber-200 p-3">
                    ...
                    <div className="space-y-2">
                      ...
                      {validation.details.dependentTransactions.slice(0, 3).map((dep) => (
                        <div key={dep.id} className="bg-card rounded border border-amber-200 p-2">
                          <div className="flex items-center justify-between">
                  ...
                  <div className="space-y-2">
                    {validation.details.dependentTransactions.map((dep) => (
                      <div key={dep.id} className="bg-card rounded border border-amber-200 p-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            {dep.transactionType === 'SHIP' ? (
                          ...
                          <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${
                            dep.transactionType === 'SHIP' ? 'bg-cyan-100 text-cyan-800' : 'bg-muted text-foreground'
                          }`}>
                            {dep.transactionType}
                          </span>
                        </div>
          ...
          {/* Footer */}
          <div className="bg-secondary px-6 py-4 border-t flex justify-end gap-3">
            <button
              onClick={onClose}
              disabled={isDeleting}
              className="px-4 py-2 text-sm font-medium text-foreground bg-card border border-border rounded-md hover:bg-secondary disabled:opacity-50"
            >
              Cancel
            </button>
            {validation.canDelete && (
