              ...
              <div>
                ...
                {cost.id === 'other' && (
                  <input
                    ...
                    className="mt-1 w-full px-3 py-2 border border-border rounded-md text-sm focus:ring-2 focus:ring-primary"
                  />
                )}
              </div>
        ...
        <div className="p-6 space-y-4">
          {costs.filter(c => c.costType === 'carton' || c.costType === 'pallet').map(cost => (
            <div key={cost.id} className="grid grid-cols-5 gap-4 items-center">
              ...
              <div className="text-center">
                <span className="text-sm text-muted-foreground">{cost.quantity} × {formatCurrency(cost.unitRate)}</span>
              </div>
