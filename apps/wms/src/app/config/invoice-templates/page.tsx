...
export default function InvoiceTemplatesPage() {
  ...
  useEffect(() => {
    ...
    if (!session) {
      window.location.href = buildCentralLoginUrl('/config/invoice-templates')
      return
    }
            ...
            <div key={template.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <p className="text-sm text-foreground">{template.warehouse.name}</p>
                </div>
                {template.isDefault && (
              ...
              <div className="space-y-2">
                ...
                <div className="text-sm">
                  <span className="text-muted-foreground">Active Cost Types:</span>
                  <div className="flex flex-wrap gap-1 mt-1">
                    {Object.entries(template.costMappings)
                      .filter(([_, mapping]) => mapping.enabled)
                      .map(([key, mapping]) => (
                        <span key={key} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-muted text-foreground">
                          {mapping.category}
                        </span>
                      ))
                    }
