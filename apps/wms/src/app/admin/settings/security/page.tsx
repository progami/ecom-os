...
export default function SecuritySettingsPage() {
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      toast.error(_error instanceof Error ? _error.message : 'Failed to save security settings')
    } finally {
      setSaving(false)
    }
  }
            ...
            {settings.ipWhitelist.length > 0 && (
              <div className="space-y-2">
                {settings.ipWhitelist.map((ip) => (
                  <div key={ip} className="flex items-center justify-between p-2 bg-secondary rounded">
