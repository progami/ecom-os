...
export default function AdminDashboardPage() {
 ...
 {/* Demo Data Toggle */}
 <label className="flex items-center gap-2 px-2 py-1 sm:px-3 sm:py-1.5 md:px-4 md:py-2 border rounded-lg hover:bg-slate-50 transition-colors cursor-pointer min-h-[44px]">
 <input
 type="checkbox"
 checked={useDemoData}
 ...
 className="p-2 sm:p-2.5 border rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50 min-w-[44px] min-h-[44px]"
 >
 ...
 <p className="text-sm text-cyan-700 ">Total SKUs</p>
 <p className="text-2xl font-bold text-cyan-900 ">1,247</p>
 <p className="text-xs text-cyan-600 ">+12% from last month</p>
 </div>
 ...
 <div className="border rounded-lg p-4 bg-gradient-to-br from-brand-teal-50 to-brand-teal-100 ">
 <p className="text-sm text-brand-teal-700 ">Avg. Turnover</p>
 <p className="text-2xl font-bold text-brand-teal-900 ">4.2x</p>
 <p className="text-xs text-brand-teal-600 ">Industry leading</p>
 </div>
 <div className="border rounded-lg p-4 bg-gradient-to-br from-orange-50 to-orange-100 ">
 <p className="text-sm text-orange-700 ">Fill Rate</p>
 ...
 <YAxis />
 <Tooltip 
 formatter={(value: number) => [`${value.toLocaleString()} cartons`, 'Inventory']}
 />
 <Area
 type="monotone"
 dataKey="inventory"
 ...
 <Tooltip formatter={(value: number) => [`${value.toLocaleString()} units`, 'Inventory']} />
 </PieChart>
 </ResponsiveContainer>
 </div>
 {useDemoData && (
...
function EnhancedDashboardCard({
 ...
 purple: 'text-brand-teal-600 bg-brand-teal-100 ',
...
function SystemAction({ title, description, icon: Icon, onClick, loading, danger }: SystemActionProps) {
 ...
 danger ? 'bg-red-100 ' : 'bg-slate-100 group-hover:bg-primary/10'
 }`}>
