...
export async function getPurchaseOrders() {
  ...
  const visibleStatuses: PurchaseOrderStatus[] = [
...
export async function ensurePurchaseOrderForTransaction(
  ...
  if (input.purchaseOrderId) {
    ...
    if (
      ...
      throw new ConflictError(
        'Transactions cannot be linked to a closed or cancelled purchase order'
      )
    }
  }
  ...
  const lineStatus =
    nextPostedQuantity >= nextQuantity
...
export async function validateDocumentsForTransition(
  ...
  for (const note of movementNotes) {
    if (
      note.attachments &&
      typeof note.attachments === 'object' &&
    ...
    for (const line of note.lines) {
      if (
        line.attachments &&
        typeof line.attachments === 'object' &&
        !Array.isArray(line.attachments)
      ) {
...
export async function transitionPurchaseOrderStatus(
  ...
  if (targetStatus === PurchaseOrderStatus.POSTED) {
    ...
    const { postedOrder, createdTransactions } = await prisma.$transaction(async tx => {
      ...
      const allLinesPosted = freshOrder.lines.every(
        line => line.status === PurchaseOrderLineStatus.POSTED
      )
      if (!allLinesPosted) {
          ...
          const createdTx = await tx.inventoryTransaction.create({
            data: {
              ...
              storagePalletsIn: isInbound
                ? Math.ceil(
                    line.quantity / Math.max(1, line.storageCartonsPerPallet ?? unitsPerCarton)
                  )
                : 0,
              ...
              shipName: !isInbound
                ? (note.referenceNumber ?? freshOrder.counterpartyName ?? null)
                : null,
              trackingNumber: null,
      ...
      if (transactionType === TransactionType.RECEIVE || transactionType === TransactionType.SHIP) {
        ...
        try {
          ledgerEntries = buildTacticalCostLedgerEntries({
            transactionType,
            receiveType:
              transactionType === TransactionType.RECEIVE ? freshOrder.receiveType : null,
  ...
  if (
    order.status === PurchaseOrderStatus.AWAITING_PROOF &&
    targetStatus === PurchaseOrderStatus.REVIEW
  ) {
