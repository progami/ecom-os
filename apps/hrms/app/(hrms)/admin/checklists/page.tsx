...
import { ChecklistTemplatesApi, EmployeesApi, HrmsSettingsApi, type ChecklistLifecycleType, type ChecklistOwnerType, type ChecklistTemplateItemInput } from '@/lib/api-client'
...
export default function AdminChecklistsPage() {
  ...
  const [templates, setTemplates] = useState<Awaited<ReturnType<typeof ChecklistTemplatesApi.list>>['items']>([])

  const [employees, setEmployees] = useState<Array<{ id: string; employeeId: string; firstName: string; lastName: string }>>([])
  const [settings, setSettings] = useState<Awaited<ReturnType<typeof HrmsSettingsApi.get>> | null>(null)
  ...
  const [newItems, setNewItems] = useState<ChecklistTemplateItemInput[]>([
    { title: 'Create accounts and access', ownerType: 'IT', dueOffsetDays: 0, evidenceRequired: false, dependsOnIndex: null },
    { title: 'Prepare workstation', ownerType: 'IT', dueOffsetDays: 0, evidenceRequired: false, dependsOnIndex: 0 },
    { title: 'Welcome + first-day agenda', ownerType: 'MANAGER', dueOffsetDays: 0, evidenceRequired: false, dependsOnIndex: null },
  ])
  ...
  async function load() {
    ...
    try {
      ...
      setEmployees((employeesRes.items ?? []).map((e) => ({ id: e.id, employeeId: e.employeeId, firstName: e.firstName, lastName: e.lastName })))
    } catch (e) {
  ...
  async function saveSettings(next: { defaultHROwnerId?: string | null; defaultITOwnerId?: string | null }) {
    setSaving(true)
    setError(null)
    try {
  ...
  return (
    <>
      <ListPageHeader
        title="Checklists"
        description="Configure onboarding/offboarding checklists and default owners."
        ...
        <div className="space-y-8">
          <Card padding="md">
            ...
            <p className="text-xs text-gray-500 mt-1">
              Used when checklist items are assigned to HR or IT. Pick a single owner for each queue (v2 will add role-based queues).
            </p>
                    ...
                    {templates.map((t) => (
                      <tr key={t.id}>
                        <td className="py-3 pr-4 font-medium text-gray-900">
                          <Link href={`/admin/checklists/templates/${t.id}`} className="hover:underline">
                            {t.name}
                          </Link>
                        </td>
                        <td className="py-3 pr-4 text-gray-700">{lifecycleLabel(t.lifecycleType)}</td>
                        ...
                        <td className="py-3 pr-4">
                          <span className={t.isActive ? 'text-xs rounded-full bg-emerald-50 text-emerald-700 px-2 py-0.5' : 'text-xs rounded-full bg-gray-100 text-gray-700 px-2 py-0.5'}>
                            {t.isActive ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="py-3 pr-0 text-right">
                          <div className="inline-flex items-center gap-2">
                            <Button href={`/admin/checklists/templates/${t.id}`} variant="secondary">
                              View
                            </Button>
                            <Button
                              variant="secondary"
          ...
          <Card padding="md">
            ...
            <p className="text-xs text-gray-500 mt-1">Start simple; iterate as workflows stabilize.</p>
            ...
            <div className="mt-6 space-y-3">
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        const next = [...newItems]
                        next.splice(idx, 1)
                        setNewItems(next.length ? next : [{ title: '', ownerType: 'HR', dueOffsetDays: 0 }])
                      }}
              ...
              <div className="flex items-center justify-between gap-2">
                <Button
                  variant="secondary"
                  onClick={() => setNewItems([...newItems, { title: '', ownerType: 'HR', dueOffsetDays: 0 }])}
                >
                  Add item
                </Button>
