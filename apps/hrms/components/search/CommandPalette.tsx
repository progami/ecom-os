        ...
        <div className="rounded-2xl border border-border bg-card shadow-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border">
            ...
            <input
              ...
              placeholder="Search…"
              className="mt-3 h-11 w-full rounded-xl border border-border bg-card px-3 text-sm text-foreground outline-none focus:border-accent"
            />
          </div>
          ...
          <div className="max-h-[60vh] overflow-y-auto">
            {loading ? (
              ...
              <div className="px-4 py-4 text-sm text-muted-foreground">Type at least 2 characters.</div>
            ) : results.length === 0 ? (
              <div className="px-4 py-4 text-sm text-muted-foreground">No results.</div>
            ) : (
              <div className="py-2">
                {grouped.map((g) => (
                      ...
                      {g.items.map((r) => {
                        ...
                        return (
                          <button
                            ...
                            className={
                              selected
                                ? 'w-full text-left rounded-xl bg-accent/5 border border-accent/20 px-3 py-2'
                                : 'w-full text-left rounded-xl hover:bg-muted/50 px-3 py-2'
                            }
                          ...
                          >
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0">
                                ...
                                {r.subtitle ? <div className="text-xs text-muted-foreground truncate mt-0.5">{r.subtitle}</div> : null}
                              </div>
                              <span className="text-[10px] font-semibold text-muted-foreground rounded-full bg-muted px-2 py-0.5 shrink-0">
                                {typeBadge(r.type)}
                              </span>
                            </div>
                          </button>
          ...
          <div className="px-4 py-3 border-t border-border text-xs text-muted-foreground flex items-center justify-between gap-3">
