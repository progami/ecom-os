...
import { WmsCostCategory as CostCategory } from '@prisma/client';
...
export async function calculateStorageCosts(
  ...
  const storageEntries = await prisma.wmsStorageLedger.findMany({
    where: {
      warehouseId,
      billingPeriodStart: {
...
export async function calculateTransactionCosts(
  ...
  const costRates = await prisma.wmsCostRate.findMany({
    where: {
      warehouseId,
      costCategory: {
