...
export async function calculateStorageLedger(
  ...
  const activeCombinations = await prisma.wmsInventoryBalance.findMany({
    where: {
      ...warehouseFilter,
      currentCartons: { gt: 0 }
    },
  ...
  for (const monday of mondays) {
    ...
    for (const combo of activeCombinations) {
      ...
      if (isAmazonWarehouse) {
        ...
        storageRate = await prisma.wmsCostRate.findFirst({
          where: {
      ...
      try {
        await prisma.wmsStorageLedger.upsert({
          where: { slId },
          update: {
...
async function getBatchPalletConfig(
  ...
  const firstReceive = await prisma.wmsInventoryTransaction.findFirst({
    where: {
      warehouseId,
      skuId,
      batchLot,
