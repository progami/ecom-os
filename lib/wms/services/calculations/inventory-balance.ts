...
export async function updateInventoryBalances(warehouseId?: string) {
  ...
  const combinations = await prisma.wmsInventoryTransaction.groupBy({
  ...
  for (const combo of combinations) {
    ...
    const sku = await prisma.wmsSku.findUnique({
    ...
    if (!storageCartonsPerPallet || !shippingCartonsPerPallet) {
      const warehouseConfig = await prisma.wmsWarehouseSkuConfig.findFirst({
        where: {
          warehouseId: combo.warehouseId,
          skuId: combo.skuId,
          OR: [
    ...
    await prisma.wmsInventoryBalance.upsert({
      where: {
  ...
  await prisma.wmsInventoryBalance.deleteMany({
    where: {
      ...warehouseFilter,
      currentCartons: 0
    }
...
export async function getInventorySummary(warehouseId?: string) {
  ...
  const summary = await prisma.wmsInventoryBalance.aggregate({
    where: warehouseFilter,
    _sum: {
      currentCartons: true,
      currentPallets: true,
  ...
  const uniqueSkus = await prisma.wmsInventoryBalance.groupBy({
    by: ['skuId'],
    where: warehouseFilter,
  })
...
export async function getInventoryMovements(
  ...
  const movements = await prisma.wmsInventoryTransaction.findMany({
    where: {
      ...warehouseFilter,
      transactionDate: {
        gte: startDate,
