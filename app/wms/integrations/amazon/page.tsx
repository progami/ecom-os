  ...
  useEffect(() => {
    const fetchAndSyncInventory = async () => {
      ...
      try {
        ...
        const response = await fetch('/api/v1/wms/amazon/inventory-comparison')
        if (response.ok) {
          ...
          const syncResponse = await fetch('/api/v1/wms/amazon/sync', {
            method: 'POST',
          ...
          if (syncResponse.ok) {
            ...
            if (result.synced > 0) {
              ...
              const refreshResponse = await fetch('/api/v1/wms/amazon/inventory-comparison')
