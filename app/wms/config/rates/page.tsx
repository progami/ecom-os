...
export default function AdminRatesPage() {
  ...
  const fetchData = async () => {
    try {
      // Fetch rates
      const ratesResponse = await fetch('/api/v1/wms/settings/rates')
      if (ratesResponse.ok) {
      ...
      // Fetch warehouses
      const warehouseResponse = await fetch('/api/v1/wms/warehouses')
                      ...
                      <tr key={rate.id} className="hover:bg-gray-50">
                        ...
                        {session?.user.role === 'admin' && (
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <Link
                              href={`/wms/config/rates/${rate.id}/edit`}
