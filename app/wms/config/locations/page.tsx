...
export default function WarehouseSettingsPage() {
  ...
  const fetchWarehouses = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/wms/warehouses?${params}`)
  ...
  const handleEdit = (warehouseId: string) => {
    router.push(`/wms/config/locations/${warehouseId}/edit`)
  }
  ...
  const handleDelete = async (warehouse: Warehouse) => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/warehouses?id=${warehouse.id}`, {
        method: 'DELETE'
      })
