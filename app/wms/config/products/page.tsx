...
export default function AdminSkusPage() {
  ...
  const fetchSkus = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/wms/skus?${params}`)
      if (!response.ok) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/skus?id=${skuToDelete.id}`, {
        method: 'DELETE'
      })
  ...
  const handleToggleActive = async (sku: SKU) => {
    try {
      const response = await fetch(`/api/v1/wms/skus?id=${sku.id}`, {
        method: 'PATCH',
  ...
  const handleEditSKU = (skuId: string) => {
    router.push(`/wms/config/products/${skuId}/edit`)
  }
                  ...
                  <td colSpan={8} className="px-6 py-12">
                    <EmptyState
                      ...
                      action={!searchTerm && !showInactive ? {
                        label: "Add First SKU",
                        onClick: () => router.push('/wms/config/products/new')
                      } : undefined}
                    />
                  </td>
                </tr>
