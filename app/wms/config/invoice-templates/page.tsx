...
export default function InvoiceTemplatesPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch warehouses
      const warehouseRes = await fetch('/api/v1/wms/warehouses')
      if (warehouseRes.ok) {
      ...
      // Fetch templates
      const templateRes = await fetch('/api/v1/wms/warehouse-configs/invoice-templates')
      if (templateRes.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingTemplate 
        ? `/api/v1/wms/warehouse-configs/invoice-templates/${editingTemplate.id}`
        : '/api/v1/wms/warehouse-configs/invoice-templates'
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/warehouse-configs/invoice-templates/${id}`, {
        method: 'DELETE'
      })
  ...
  const handleCopy = async (template: InvoiceTemplate) => {
    ...
    try {
      const response = await fetch('/api/v1/wms/warehouse-configs/invoice-templates', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
