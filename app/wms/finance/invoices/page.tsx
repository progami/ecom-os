...
export default function FinanceInvoicesPage() {
  ...
  const fetchInvoices = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/v1/wms/invoices?${params}`)
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch('/api/v1/wms/invoices/upload', {
        method: 'POST',
        body: formData
      })
      ...
      if (!response.ok) {
        if (data.requiresManualEntry) {
          ...
          router.push('/wms/finance/invoices/new?manual=true&filename=' + encodeURIComponent(data.fileName))
        } else {
  ...
  const handleViewInvoice = (invoiceId: string) => {
    router.push(`/wms/finance/invoices/${invoiceId}`)
  }

  const handleProcessInvoice = (invoiceId: string) => {
    router.push(`/wms/finance/reconciliation?invoiceId=${invoiceId}`)
  }
  ...
  const handlePayInvoice = async (invoiceId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/invoices/${invoiceId}/accept`, {
        method: 'POST',
  ...
  const handleDisputeInvoice = async (invoiceId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/invoices/${invoiceId}/dispute`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/wms/export?${params}`)
