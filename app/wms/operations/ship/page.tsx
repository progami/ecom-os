...
export default function WarehouseShipPage() {
  ...
  const fetchLastShipmentData = async () => {
    try {
      const response = await fetch('/api/v1/wms/transactions/ledger?transactionType=SHIP&limit=1')
      if (response.ok) {
  ...
  const fetchSkus = async () => {
    try {
      setSkuLoading(true)
      const response = await fetch('/api/v1/wms/skus')
      if (response.ok) {
        const data = await response.json()
  ...
  const fetchInventory = async (warehouseId?: string) => {
    try {
      const url = warehouseId 
        ? `/api/v1/wms/inventory/balances?warehouseId=${warehouseId}`
        : '/api/v1/wms/inventory/balances'
...
function EmailModal({ shipmentData, onClose }: { shipmentData: any; onClose: () => void }) {
  ...
  const generateEmail = async () => {
    setLoading(true)
    try {
      const response = await fetch('/api/v1/wms/inventory/shipments/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
