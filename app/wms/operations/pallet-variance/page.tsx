...
export default function PalletVariancePage() {
  ...
  const fetchVariances = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/v1/wms/operations/pallet-variance')
      if (response.ok) {
  ...
  const handleCreateAdjustment = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetch(`/api/v1/wms/operations/pallet-variance/${selectedVariance.id}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            status: 'RESOLVED',
  ...
  const handleExport = () => {
    window.open('/api/v1/wms/operations/pallet-variance/export', '_blank')
