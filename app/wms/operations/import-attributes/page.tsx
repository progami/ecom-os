...
import { EmptyState } from '@/components/wms/shared/empty-state'
...
export default function ImportAttributesPage() {
  ...
  const [selectedTransaction, setSelectedTransaction] = useState<WmsTransaction | null>(null)
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/v1/wms/transactions?includeAttachments=true&limit=500')
      if (response.ok) {
  ...
  const getMissingFields = (transaction: WmsTransaction): string[] => {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/transactions/${selectedTransaction.id}/attachments`, {
        method: 'POST',
        body: formData
      })
  ...
  const handleAttributeUpdate = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/wms/transactions/${selectedTransaction.id}/attributes`, {
        method: 'PATCH',
        ...
        <PageHeader
          ...
          actions={
            <button
              ...
              onClick={() => window.open('/api/v1/wms/export/missing-attributes', '_blank')}
            >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => {
                            ...
                            router.push(`/wms/operations/transactions/${transaction.id}`)
                          }}
