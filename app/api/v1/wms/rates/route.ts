...
// GET /api/v1/wms/rates - List cost rates
export async function GET(req: NextRequest) {
  try {
    ...
    const rates = await prisma.wmsCostRate.findMany({
      where,
      include: {
        warehouse: {
          select: {
...
// POST /api/v1/wms/rates - Create new rate
export async function POST(req: NextRequest) {
  try {
    ...
    const overlapping = await prisma.wmsCostRate.findFirst({
      where: {
    ...
    const rate = await prisma.wmsCostRate.create({
      data: {
        ...validatedData,
...
// PATCH /api/v1/wms/rates - Update rate
export async function PATCH(req: NextRequest) {
  try {
    ...
    const rate = await prisma.wmsCostRate.findUnique({
      where: { id: rateId },
      include: {
        _count: {
          select: {
    ...
    const updatedRate = await prisma.wmsCostRate.update({
      where: { id: rateId },
      data: {
        ...validatedData,
...
// DELETE /api/v1/wms/rates - Delete rate
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (rate._count.calculatedCosts > 0) {
      ...
      const endedRate = await prisma.wmsCostRate.update({
        where: { id: rateId },
        data: { 
          endDate: new Date(),
          updatedAt: new Date()
      ...
      // Delete the rate
      await prisma.wmsCostRate.delete({
        where: { id: rateId }
      })
