...
// GET /api/v1/wms/skus/[id] - Get a single SKU by ID
...
// PUT /api/v1/wms/skus/[id] - Update a SKU
...
// DELETE /api/v1/wms/skus/[id] - Delete or deactivate a SKU
export async function DELETE(
  ...
  try {
    ...
    if (sku._count.inventoryBalances > 0 || sku._count.warehouseConfigs > 0) {
      const deactivatedSku = await prisma.wmsSku.update({
        where: { id: params.id },
        data: { isActive: false }
      })
